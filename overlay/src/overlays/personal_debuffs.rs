...
//!
//! Displays active debuffs on the local player as a horizontal row of icons.
//! Shows debuffs from NPCs/bosses with source name and cleansable highlighting.
...
/// A single debuff entry for display
#[derive(Debug, Clone)]
pub struct PersonalDebuff {
    /// Effect ID for identification
    pub effect_id: u64,
    /// Ability ID for icon lookup
    pub icon_ability_id: u64,
    /// Display name of the debuff
    pub name: String,
    ...
    pub stacks: u8,
    /// Is this debuff cleansable?
    pub is_cleansable: bool,
    /// Source entity name (boss/NPC that applied it)
...
/// Data sent from service to personal debuffs overlay
#[derive(Debug, Clone, Default)]
pub struct PersonalDebuffsData {
    pub debuffs: Vec<PersonalDebuff>,
}

/// Configuration for personal debuffs overlay
#[derive(Debug, Clone)]
...
/// Personal debuffs overlay - horizontal row of debuff icons
pub struct PersonalDebuffsOverlay {
    frame: OverlayFrame,
    config: PersonalDebuffsConfig,
    background_alpha: u8,
    data: PersonalDebuffsData,
...
impl PersonalDebuffsOverlay {
    /// Create a new personal debuffs overlay
    pub fn new(
        ...
        frame.set_label("Personal Debuffs");

        Ok(Self {
            frame,
            config,
            background_alpha,
            data: PersonalDebuffsData::default(),
    ...
    /// Update the config
    pub fn set_config(&mut self, config: PersonalDebuffsConfig) {
        self.config = config;
    }
    ...
    pub fn set_data(&mut self, data: PersonalDebuffsData) {
        let icon_size = self.frame.scaled(self.config.icon_size as f32) as u32;

        for debuff in &data.debuffs {
            if let Some(ref icon_arc) = debuff.icon {
                let cache_key = (debuff.icon_ability_id, icon_size);
    ...
    pub fn render(&mut self) {
        ...
        for debuff in self.data.debuffs.iter().take(max_display) {
            // Cleansable highlight border (thicker, purple)
            if self.config.highlight_cleansable && debuff.is_cleansable {
            ...
            let cache_key = (debuff.icon_ability_id, icon_size_u32);
            let has_icon = if debuff.show_icon {
                if let Some(scaled_icon) = self.icon_cache.get(&cache_key) {
                    ...
                    true
                } else if let Some(ref icon_arc) = debuff.icon {
            ...
            if !has_icon {
                ...
                let bg_color = color_from_rgba(debuff.color);
            ...
            // Border (red for debuffs)
            let border_color = colors::effect_debuff();
            self.frame.stroke_rounded_rect(
                x,
                y,
                icon_size,
            ...
            if self.config.stack_priority && debuff.stacks >= 1 {
                // STACK PRIORITY: Big stacks centered
                let stack_text = format!("{}", debuff.stacks);
                ...
                if self.config.show_countdown && debuff.total_secs > 0.0 {
                    let time_text = debuff.format_time();
            ...
            if self.config.show_effect_names {
                ...
                let name = truncate_name(&debuff.name, 8);
...
impl Overlay for PersonalDebuffsOverlay {
    fn update_data(&mut self, data: OverlayData) -> bool {
        if let OverlayData::PersonalDebuffs(debuffs_data) = data {
            let was_empty = self.data.debuffs.is_empty();
            let is_empty = debuffs_data.debuffs.is_empty();
            self.set_data(debuffs_data);
            !(was_empty && is_empty)
        } else {
            false
        }
    ...
    fn update_config(&mut self, config: OverlayConfigUpdate) {
        if let OverlayConfigUpdate::PersonalDebuffs(cfg, alpha) = config {
            self.set_config(cfg);
    ...
    fn render(&mut self) {
        PersonalDebuffsOverlay::render(self);
    }
