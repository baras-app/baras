...
impl BossHealthOverlay {
    ...
    /// (overrides the plain marker-text row). `None` falls back to the old
    /// marker-text-only path for backward compatibility.
    fn entry_height(
        &self,
        ...
        if let Some(row_h) = icon_row_height {
            ...
            // No icons — keep original marker-text-only height
        ...
        let total_needed: f32 = padding * 2.0
            + entries
                ...
                .map(|e| {
                    ...
                    let icon_row = (has_icons || Self::next_marker(e).is_some())
                        .then_some(icon_row_h);
                    self.entry_height(
                        e,
                        bar_height,
                        label_height,
        ...
        for entry in &entries {
            ...
            if entry_icons.is_some_and(|v| !v.is_empty()) {
                ...
                for icon_entry in icons {
                    // Draw icon or fallback colored square
                ...
                // No icons — keep original centered marker-label behavior
                let marker_font_size = label_font_size * 0.85;
                let marker_label = format!("{}% {}", hp_pct as u32, label);
                let marker_x = padding + (hp_pct / 100.0) * content_width;
