...
fn test_parse_details_damage_basic() {
    ...
    let result = parse_details(input, "Damage");
    assert!(result.is_some());
...
fn test_parse_details_heal_basic() {
    let input = " (3500) <1750>";
    let result = parse_details(input, "Heal");
    assert!(result.is_some());
...
fn test_parse_details_modify_charges() {
    ...
    let result = parse_details(input, "ModifyCharges");
    assert!(result.is_some());
...
fn test_parse_details_apply_effect_with_charges() {
    ...
    let result = parse_details(input, "ApplyEffect");
    assert!(result.is_some());
...
fn test_parse_details_default() {
    let input = "";
    let result = parse_details(input, "RemoveEffect");
    assert!(result.is_some());
