...
- Two-pass borrow pattern for phase transitions (immutable find, mutable apply)
- Toast notifications for error feedback (5s normal, 7s critical)
- Accept ~30% clone reduction as achievable (50% target was optimistic given Rust ownership)

**Issues Resolved:**

- 320+ `.unwrap()`/`.expect()` calls eliminated from production code
- 382 `eprintln!` calls migrated to structured tracing
- UI no longer freezes on backend errors
- Hot path clones reduced ~30% average

**Technical Debt Incurred:**

- Pre-existing clippy warnings (30+) not addressed (separate cleanup)
- Overlay example `new_overlays.rs` has stale API references
- Some test files still use `unwrap()` (acceptable per policy)
