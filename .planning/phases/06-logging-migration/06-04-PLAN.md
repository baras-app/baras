...
<task type="auto">
  ...
  <files>overlay/src/main.rs</files>
  <action>
Add tracing subscriber initialization to overlay binary for standalone debugging, then migrate eprintln! calls.

**Tracing init (add at start of main()):**
```rust
...
**Migrate eprintln! calls (~13 calls):**
...
`cargo run -p baras-overlay -- --metric` runs and shows tracing output on errors
  ...
  <done>Overlay binary has tracing init, all eprintln! converted</done>
</task>
...
<success_criteria>
1. Overlay binary initializes tracing subscriber for standalone mode
2. All eprintln! in main.rs converted to tracing::error! for failures
3. All eprintln! in wayland.rs converted with appropriate levels
4. Windows win_log! macro converted to use tracing::debug!
5. All eprintln! in icons.rs converted
6. Phase success: `grep -rn "eprintln!" --include="*.rs" | grep -v "_tests.rs" | grep -v "validate/"` returns nothing for core/, overlay/, app/src-tauri/
</success_criteria>
