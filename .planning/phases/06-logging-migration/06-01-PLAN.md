...
must_haves:
  truths:
    - "App writes logs to ~/.config/baras/logs/baras.log"
...
1. `pub fn init() -> Option<tracing_appender::non_blocking::WorkerGuard>` that:
   ...
   - Creates log directory at `dirs::config_dir()/baras/logs`
   - Creates `BasicRollingFileAppender` with:
     - Path: `{log_dir}/baras.log`
...
Handle errors gracefully - if log directory creation fails, fall back to stdout-only logging.
  ...
  <verify>
1. `cargo build -p app`
2. Run app, check `~/.config/baras/logs/baras.log` exists
...
<verification>
- [ ] `ls ~/.config/baras/logs/baras.log` shows file exists after app run
