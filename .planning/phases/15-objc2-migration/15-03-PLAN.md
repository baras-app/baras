...
<task type="auto">
  <name>Task 4: Migrate simple methods (set_position, set_size, set_click_through, commit, Drop)</name>
  <files>overlay/src/platform/macos.rs</files>
  <action>
Update the simpler methods in MacOSOverlay to use objc2-app-kit:
...
```
  ...
  <verify>
Run `cargo check -p baras-overlay --target aarch64-apple-darwin` to verify these methods compile.
  </verify>
  <done>
Simple methods (set_position, set_size, set_click_through, commit, Drop) migrated to objc2-app-kit.
  </done>
</task>

<task type="auto">
  <name>Task 5: Migrate complex methods (poll_events, get_monitors)</name>
  <files>overlay/src/platform/macos.rs</files>
  <action>
Update the more complex methods that require careful event/screen handling:
...
Run `cargo check -p baras-overlay --target aarch64-apple-darwin` to verify all methods compile.
  ...
  <done>
Complex methods (poll_events, get_monitors) migrated to objc2-app-kit with proper event handling and screen enumeration.
  </done>
</task>
