...
use super::{
    AbilitySelector, DefinitionSet, EffectCategory, EffectDefinition, EffectSelector,
    EffectTracker, EntityFilter,
};
...
fn test_effect_removed_marks_inactive() {
    ...
    assert_eq!(effects.len(), 1);
    assert!(
        effects[0].removed_at.is_some(),
        "Effect should be marked removed"
    );
}
...
fn test_entity_death_clears_effects() {
    ...
    assert!(
        effects[0].removed_at.is_some(),
        "Effect should be marked removed on death"
    );
}
...
fn test_persist_past_death_keeps_effect() {
    ...
    let effects: Vec<_> = tracker.active_effects().collect();
    assert!(
        effects[0].removed_at.is_none(),
        "Persistent effect should survive death"
    );
}
...
fn test_combat_end_clears_combat_only_effects() {
    ...
    let persistent_effect = effects
        .iter()
        .find(|e| e.definition_id == "persistent_buff");

    assert!(
        combat_effect.unwrap().removed_at.is_some(),
        "Combat-only effect should be removed"
    );
    assert!(
        persistent_effect.unwrap().removed_at.is_none(),
        "Persistent effect should survive"
    );
}
...
fn test_area_entered_clears_all_effects() {
    ...
    assert!(
        effects[0].removed_at.is_some(),
        "Effect should be cleared on area change"
    );
}
...
fn test_ability_activated_refreshes_effect() {
    ...
    let effects: Vec<_> = tracker.active_effects().collect();
    assert!(
        effects[0].last_refreshed_at > first_refreshed,
        "Effect should be refreshed"
    );
}
...
fn test_player_initialized_sets_local_player() {
    ...
    assert_eq!(effects.len(), 1);
    assert!(
        effects[0].is_from_local_player,
        "Effect should be marked as from local player"
    );
}
...
fn test_boss_filter_uses_encounter_context() {
    ...
    assert_eq!(
        ...
        0,
        "Should not match - no encounter context"
    );
    ...
    assert_eq!(
        tracker.active_effects().count(),
        1,
        "Should match with encounter context"
    );
}
...
fn test_live_mode_required_for_tracking() {
    ...
    assert!(
        !tracker.has_active_effects(),
        "Should not track in historical mode"
    );
}
...
fn test_filter_local_player() {
    ...
    assert_eq!(
        tracker.active_effects().count(),
        1,
        "Local player source should match"
    );
}
...
fn test_filter_local_player_rejects_other() {
    ...
    assert_eq!(
        tracker.active_effects().count(),
        0,
        "Other player source should not match LocalPlayer filter"
    );
}
...
fn test_filter_other_players() {
    ...
    assert_eq!(
        tracker.active_effects().count(),
        1,
        "Local player should not match OtherPlayers"
    );
}
...
fn test_filter_any_player() {
    ...
    assert_eq!(
        tracker.active_effects().count(),
        2,
        "Both players should match AnyPlayer"
    );
}
...
fn test_filter_any_npc() {
    ...
    assert_eq!(
        tracker.active_effects().count(),
        1,
        "Player should not match AnyNpc"
    );
}
...
fn test_filter_npc_except_boss() {
    ...
    assert_eq!(
        tracker.active_effects().count(),
        0,
        "Boss should not match NpcExceptBoss"
    );
...
fn test_filter_companion() {
    ...
    assert_eq!(
        tracker.active_effects().count(),
        1,
        "Player should not match AnyCompanion"
    );
}
...
fn test_filter_any_player_or_companion() {
    ...
    assert_eq!(
        tracker.active_effects().count(),
        2,
        "Both player and companion should match"
    );
...
fn test_non_matching_effect_id_ignored() {
    ...
    assert_eq!(
        tracker.active_effects().count(),
        0,
        "Non-matching effect ID should be ignored"
    );
}
...
fn test_target_tracking_for_ability_refresh() {
    ...
    let effects: Vec<_> = tracker.active_effects().collect();
    assert!(
        effects[0].last_refreshed_at > ts,
        "Effect should be refreshed via tracked target"
    );
