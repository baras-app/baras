...
impl ChallengeTracker {
    ...
    fn end_current_phase(&mut self, timestamp: chrono::NaiveDateTime) {
        if let Some((phase_id, start_time)) = self.current_phase_start.take() {
            ...
            // Accumulate duration (phase may be re-entered)
            *self.phase_durations.entry(phase_id).or_insert(0.0) += duration_secs;
        }
    }

    /// Set the total encounter duration (called on each event or polling)
    ...
    /// Get current values snapshot (for overlay updates)
    ...
    pub fn process_damage(
        ...
        for def in &self.definitions {
            let (matches_metric, track_source) = match def.metric {
                ChallengeMetric::Damage => (true, true),      // Track dealer (source)
                ChallengeMetric::DamageTaken => (true, false), // Track recipient (target)
                _ => (false, false),
            };
            ...
            if def.matches(ctx, Some(source), Some(target), Some(ability_id), None) {
                if let Some(val) = self.values.get_mut(&def.id) {
                    val.value += damage;
                    val.event_count += 1;
                    // Track per-player based on metric type
    ...
    pub fn process_healing(
        ...
        for def in &self.definitions {
            let (matches_metric, track_source) = match def.metric {
                ChallengeMetric::Healing => (true, true),      // Track healer (source)
                ChallengeMetric::HealingTaken => (true, false), // Track recipient (target)
                _ => (false, false),
            };
        ...
        for def in &self.definitions {
            ...
            if def.matches(ctx, Some(source), Some(target), Some(ability_id), None) {
                if let Some(val) = self.values.get_mut(&def.id) {
                    val.value += 1;
                    val.event_count += 1;
                    // Track by source player (who activated the ability)
                    if source.is_player {
        ...
        for def in &self.definitions {
            ...
            if def.matches(ctx, Some(source), Some(target), None, Some(effect_id)) {
                if let Some(val) = self.values.get_mut(&def.id) {
                    val.value += 1;
                    val.event_count += 1;
                    // Track by source player (who applied the effect)
                    if source.is_player {
                        *val.by_player.entry(source.entity_id).or_insert(0) += 1;
                    }
                    updated.push(def.id.clone());
    ...
    pub fn process_death(
        ...
        for def in &self.definitions {
            ...
            // For deaths, the entity is the "target" that died
            if def.matches(ctx, None, Some(entity), None, None) {
                if let Some(val) = self.values.get_mut(&def.id) {
                    val.value += 1;
                    val.event_count += 1;
                    // Track by player who died
                    if entity.is_player {
        ...
        for def in &self.definitions {
            ...
            if def.matches(ctx, Some(source), Some(target), None, None) {
                if let Some(val) = self.values.get_mut(&def.id) {
                    val.value += threat;
                    val.event_count += 1;
                    // Track by source player (who generated threat)
                    if source.is_player {
